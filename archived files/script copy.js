...
<!-- Make sure you have a div with id="data-chart" and class="image-wrapper" -->
<div id="scroll">
  <div class="image-wrapper" id="data-chart"></div>
  <article>
    <div class="step" data-step="0">Step 0</div>
    <div class="step" data-step="1">Step 1</div>
    <!-- add steps as needed -->
  </article>
</div>

<img id="image-a" class="show" />
<img id="image-b" />
...
<script>
  // Scrollama setup
  var container = d3.select("#scroll");
  var figure = container.select(".image-wrapper");
  var article = container.select("article");
  var step = article.selectAll(".step");
  ...
  function handleStepEnter(response) {
    ...
    if (response.index === 0) {
      ...
      document.querySelector("[data-step='9']").style.position = "sticky";
      document.querySelector("[data-step='9']").style.top = "0px";
      document.querySelector("[data-step='12']").style.position = "relative";
    } else if (response.index === 10) {
      swapImages("images/10-2005-impact.svg");
    } else if (response.index === 11) {
      swapImages("images/11-2005-deaths.svg");
      ...
      swapImages("images/12-chart.svg");
      document.querySelector("[data-step='9']").style.position = "relative";
      document.querySelector("[data-step='12']").style.position = "sticky";
      document.querySelector("[data-step='12']").style.top = "0px";
    } else if (response.index === 13 || response.index === 14) {
      swapImages("images/13-chart-labels.svg");
    }
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    // Chart dimensions
    let chartWrapper = document.getElementById("data-chart");
    ...
    d3.csv("storm-data-subregion-filter.csv").then(function(data) {
      ...
      // Y axis (use unique subregions)
      var y = d3.scaleBand()
        .domain([...new Set(filteredData.map(d => d["Subregion"]))])
        .range([height, 0]);
