...
function handleStepEnter(response) {
  ...
  if (!chartDrawn) {
    ...
    const height = 600; // or whatever fits your design
    const margin = { top: 20, right: 30, bottom: 60, left: 90 };

    d3.csv("storm-data-subregion-filter.csv").then(function(data) {
      drawStormChart(data, 2000, chartContainer, width, height, margin);

      // Year button interactivity
      document.querySelectorAll("#year-buttons button").forEach(button => {
        button.addEventListener("click", () => {
          ...
          drawStormChart(data, year, chartContainer, width, height, margin);
        });
      });
    });
  }
